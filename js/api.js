...
const TBApi = (() => {
  ...
  // ---------------- extended clinical/diagnostic context (supplementary) ----------------
  // These come from the project's wider TB-Portals feature-selection work
  // (biochemistry, DST/lab, and patient-case files) — NOT from the Wang et
  // al. paper the core 6-marker model above is calibrated to. There is no
  // trained coefficient behind them, so they are kept as a separate
  // qualitative "flags" assessment rather than blended into
  // probability_failure above — mixing an un-validated adjustment into a
  // number that's presented as the validated model's output would overstate
  // its precision. Direction/magnitude here are clinically-informed
  // defaults for demo purposes only.
  const EXT_NUMERIC = {
    crp: { label: 'C-Reactive Protein', mid: 8, scale: 15, weight: 0.10, invert: false },
    esr: { label: 'ESR', mid: 20, scale: 25, weight: 0.08, invert: false },
    ada: { label: 'Adenosine Deaminase', mid: 20, scale: 15, weight: 0.08, invert: false },
    nlr: { label: 'Neutrophil-Lymphocyte Ratio', mid: 2, scale: 2, weight: 0.08, invert: false },
    albumin: { label: 'Albumin', mid: 40, scale: 8, weight: 0.08, invert: true },
    hemoglobin: { label: 'Hemoglobin', mid: 135, scale: 25, weight: 0.06, invert: true },
    bmi: { label: 'BMI', mid: 21, scale: 4, weight: 0.05, invert: true },
  };
  const EXT_CATEGORICAL = {
    geneXpert: {
      label: 'GeneXpert result',
      points: { positive: 0.35, indeterminate: 0.10, negative: -0.15 },
    },
    culture: {
      label: 'Culture result',
      points: { positive: 0.30, pending: 0, negative: -0.15 },
    },
    resistanceType: {
      label: 'Drug-resistance type',
      points: { 'drug-susceptible': -0.10, 'mono-resistant': 0.15, mdr: 0.45, xdr: 0.65 },
    },
    comorbidity: {
      label: 'Comorbidity',
      points: { none: -0.05, diabetes: 0.15, hiv: 0.20, other: 0.05 },
    },
    lungLocalization: {
      label: 'Lung localization',
      points: { unilateral: 0, bilateral: 0.15 },
    },
  };

  function assessExtended(ext) {
    const flags = [];
    let score = 0;
    let any = false;

    Object.keys(EXT_NUMERIC).forEach((key) => {
      const spec = EXT_NUMERIC[key];
      const raw = ext ? ext[key] : null;
      if (raw === null || raw === undefined || raw === '' || Number.isNaN(Number(raw))) return;
      any = true;
      const num = Number(raw);
      let dev = (num - spec.mid) / spec.scale;
      if (spec.invert) dev = -dev;
      const clipped = Math.max(-2, Math.min(2, dev));
      const points = Math.round(spec.weight * clipped * 3 * 1000) / 1000;
      score += points;
      if (Math.abs(clipped) >= 0.4) {
        flags.push({
          label: spec.label,
          tone: points > 0 ? 'risk' : 'protective',
          note: points > 0 ? 'outside the reference range, in the direction associated with worse outcomes'
                            : 'within or better than the typical reference range',
        });
      }
    });

    Object.keys(EXT_CATEGORICAL).forEach((key) => {
      const spec = EXT_CATEGORICAL[key];
      const raw = ext ? ext[key] : null;
      if (!raw) return;
      const norm = String(raw).toLowerCase();
      if (!(norm in spec.points)) return;
      any = true;
      const points = spec.points[norm];
      score += points;
      if (points !== 0) {
        flags.push({
          label: spec.label,
          tone: points > 0 ? 'risk' : 'protective',
          note: `recorded as "${raw}"`,
        });
      }
    });

    flags.sort((a, b) => (a.tone === 'risk' ? -1 : 1) - (b.tone === 'risk' ? -1 : 1));

    const direction = !any ? 'none' : score > 0.15 ? 'elevated' : score < -0.15 ? 'reduced' : 'neutral';
    const summary = !any
      ? 'No additional clinical context provided.'
      : direction === 'elevated'
        ? 'These additional findings lean toward higher risk — consider alongside the core panel result, not in place of it.'
        : direction === 'reduced'
          ? 'These additional findings lean reassuring, but do not override the core panel result.'
          : 'These additional findings are mixed or close to typical — no strong signal either way.';

    return { any, direction, flags, summary };
  }
  ...
  return {
    ...
    async assessExtended(ext) {
      requireUser();
      await delay();
      return assessExtended(ext);
    },
    ...
    async createPatient(payload) {
      ...
      const extended = {
        crp: payload.crp, esr: payload.esr, ada: payload.ada, nlr: payload.nlr,
        albumin: payload.albumin, hemoglobin: payload.hemoglobin, bmi: payload.bmi,
        geneXpert: payload.geneXpert, culture: payload.culture,
        resistanceType: payload.resistanceType, comorbidity: payload.comorbidity,
        lungLocalization: payload.lungLocalization,
      };
      const extendedAssessment = assessExtended(extended);
      ...
      const record = {
        ...
        biomarkers,
        prediction,
        extended: extendedAssessment.any ? extended : null,
        extended_assessment: extendedAssessment.any ? extendedAssessment : null,
